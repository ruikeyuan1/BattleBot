...
//    }
    ...
    void fixIt(int flagFix) {
      if(flagFix == 0) {
        while(analogRead(rightSensor)<(approachingBLACK)) {
          rotateRight();
        }
      } else if(flagFix == 1) {
        while(analogRead(leftSensor)<(approachingBLACK)) {
          rotateLeft();
        }
      }
    }
...
//        }
        ...
        if(analogRead(leftSensor) >= (BLACKK-300)) { 
            if(!check) {
              check = true;
              goStop(150); 
            }
        ...
        if(analogRead(rightSensor) >= (BLACKK-300)) {  
            if(!check) {
              check = true;
              goStop(150); 
            }
        ...
        if(analogRead(leftSensor) < (BLACKK-300) && analogRead(rightSensor) < (BLACKK-300)) {
            check = false;
            goForward();
        }
